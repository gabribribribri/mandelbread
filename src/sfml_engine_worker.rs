...
impl SfmlEngineWorkerInternal {
    ...
    fn choose_reload_internal(&mut self) -> WorkerResult {
        ...
        match backend {
            ...
            FractalBackend::F128 => self.reload_internal_f128(),
    ...
    fn reload_internal_f128(&mut self) -> WorkerResult {
        let start = Instant::now();

        let ctx = self.ctx_rwl.read().unwrap().clone();
        let center_c128 = Complex::new(
            ctx.center.real().to_f64() as f128,
            ctx.center.imag().to_f64() as f128,
        );
        let window_c128 = Complex::new(
            ctx.window.real().to_f64() as f128,
            ctx.window.imag().to_f64() as f128,
        );
        let res_lodiv_c128 = Complex::new(
            (ctx.res.x / ctx.lodiv) as f128,
            (ctx.res.y / ctx.lodiv) as f128,
        );
        let seq_iter = ctx.seq_iter;
        let converge_distance = ctx.converge_distance as f128;
        drop(ctx);
        ...
        for y in 0..self.rrect.height {
            for x in 0..self.rrect.width {
                let c = Complex::map_pixel_value_f128(
                    res_lodiv_c128,
                    center_c128,
                    window_c128,
                    Complex::new((self.rrect.left + x) as f128, (self.rrect.top + y) as f128),
                );
                let mut n = c;
                let mut distance = 0.0f128;
                let mut iter = 0;
                while iter < seq_iter && distance <= converge_distance {
                    n.f_sq_add_f128(c);
                    distance = n.abs_sum_f128();
                    iter += 1;
                }
                if distance <= converge_distance {
                    pixels.extend_from_slice(&[0, 0, 0, 255]);
