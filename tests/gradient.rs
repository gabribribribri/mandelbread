use mandelbread::utils::{ConversionColor, distance_gradient};
#[test]
fn test_gradient_start_red() {
    // Value 0.0 should be pure Red (255, 0, 0)
    assert_eq!(distance_gradient::<0, 100>(0.0), ConversionColor(255, 0, 0));
}
...
fn test_gradient_midpoint_green() {
    ...
    assert_eq!(
        distance_gradient::<0, 100>(50.0),
        ConversionColor(0, 255, 0)
    );
}
...
fn test_gradient_end_blue() {
    ...
    assert_eq!(
        distance_gradient::<0, 100>(100.0),
        ConversionColor(0, 0, 255)
    );
    assert_eq!(
        distance_gradient::<0, 1000>(1000.0),
        ConversionColor(0, 0, 255)
    );
    assert_eq!(
        distance_gradient::<300, 1000>(1000.0),
        ConversionColor(0, 0, 255)
    );
}
...
fn test_first_segment_quarter_point() {
    ...
    assert_eq!(
        distance_gradient::<0, 100>(25.0),
        ConversionColor(128, 128, 0)
    );
    assert_eq!(
        distance_gradient::<0, 1000>(250.0),
        ConversionColor(128, 128, 0)
    );
    assert_eq!(
        distance_gradient::<500, 1500>(750.0),
        ConversionColor(128, 128, 0)
    );
}
...
fn test_second_segment_quarter_point() {
    ...
    assert_eq!(
        distance_gradient::<0, 100>(75.0),
        ConversionColor(0, 128, 128)
    );
}
...
fn test_value_just_before_midpoint() {
    ...
    assert_eq!(
        distance_gradient::<0, 100>(49.9),
        ConversionColor(1, 254, 0)
    );
}
...
fn test_value_just_after_midpoint() {
    ...
    assert_eq!(
        distance_gradient::<0, 100>(50.1),
        ConversionColor(0, 254, 1)
    );
}
...
fn test_clamping_below_zero() {
    ...
    assert_eq!(
        distance_gradient::<0, 100>(-10.0),
        ConversionColor(255, 0, 0)
    );
}
...
fn test_floating_point_precision() {
    ...
    assert_eq!(distance_gradient::<0, 100>(1.0), ConversionColor(250, 5, 0));
    assert_eq!(
        distance_gradient::<0, 100>(99.0),
        ConversionColor(0, 5, 250)
    );
}
